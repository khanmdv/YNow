...
- (NSArray *)generateTagViews{
    ...
    for (NSString *tag in sortedTags) {
        ...
        UIButton* button = [[UIButton alloc] initWithFrame:CGRectMake(center.x - size.width/2, center.y - size.height/2, size.width, size.height)];

        float r = (rand() % 10) * 0.1;
        float g = (rand() % 10) * 0.1;
        float b = (rand() % 10) * 0.1;
        
        // If it is near white gray it off
        if (r >= 0.8 & g >= 0.8 && g >= 0.8){
            r = g = b = 0.2;
        }
        
        button.titleLabel.text = tag;
        button.titleLabel.font = tagFont;
        button.titleLabel.textColor = [UIColor colorWithRed:r green:g blue:b alpha:1];
        
        tagLabel.text = tag;
        tagLabel.font = tagFont;
        tagLabel.textColor = [UIColor colorWithRed:r green:g blue:b alpha:1];
